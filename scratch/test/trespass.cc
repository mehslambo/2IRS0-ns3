...
void configureTCPSensorClients() {
	...
	factory.Set("PacketSize", UintegerValue(2000));
	...
	for (uint16_t i = 0; i < (totalNodes); i++) 
	{
		...
		factory.Set("Interval", TimeValue(Minutes(times[int(itterator)]) + Seconds(2)));
...
int main(int argc, char *argv[]) {
  	...
  	for (NodeContainer::Iterator j = wifiStaNode.Begin (); j != wifiStaNode.End (); ++j)
    {
      ...
      Vector position = mobility->GetPosition ();
      position.x = positions[int(itterator) % 12][0];
      position.y = positions[int(itterator) % 12][1];
      position.z = 3;
