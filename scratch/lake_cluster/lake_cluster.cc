...
NS_LOG_COMPONENT_DEFINE("lake_cluster");
...
// Arrange nodes based on the dimensions variable (1 for line, 2 for 2D matrix, 3 for 3D matrix)
int dimensions = 2; // Change this value to 1, 2, or 3 to switch between line, 2D matrix, or 3D matrix
...
int main(int argc, char *argv[]) {
	...
	cmd.AddValue("dimensions", "Number of dimensions", dimensions);
    ...
    PositionLogging posLogger("lake_cluster");
    posLogger.EnableLogging();


	int rows = (dimensions >= 2) ? static_cast<int>(std::sqrt(totalNodes)) : 1; // Calculate the number of rows for 2D or 3D
	int cols = (dimensions >= 2) ? (totalNodes + rows - 1) / rows : totalNodes; // Calculate the number of columns for 2D or 3D
	int layers = (dimensions == 3) ? static_cast<int>(std::cbrt(totalNodes)) : 1; // Calculate the number of layers for 3D

	int counter = 0;
	for (NodeContainer::Iterator j = wifiStaNode.Begin(); j != wifiStaNode.End(); ++j)
	{
		Ptr<MobilityModel> mobility = (*j)->GetObject<MobilityModel>();
		double x = nodeSpacing * (counter % cols); // Column index
		double y = (dimensions >= 2) ? nodeSpacing * ((counter / cols) % rows) : 0; // Row index for 2D or 3D
		double z = (dimensions == 3) ? -nodeSpacing * (counter / (cols * rows)) : -(depth); // Layer index for 3D or constant depth for 1D/2D
  ...
  for (uint32_t i = 0; i < gateways * gateways; i++)
  {
	...
	PacketLogging logger("lake_cluster", wifiStaNode, wifiApNode);
