...
PowerLoggingRaw::PowerLoggingRaw(const std::string &scenarioName,
...
std::string PowerLoggingRaw::GetNodePos(uint32_t nodeId)
{
...
void PowerLoggingRaw::PhyStateChangeCallback(std::string context, const Time start, const Time duration, const WifiPhy::State state)
{
    ...
    if (!m_logFile.is_open())
    {
        std::cout << "Log file for PhyStateChangeCallback is not open: " << m_logFilePath << std::endl;
        return;
    }
...
void PowerLoggingRaw::WriteHeader()
{
    if (m_logFile.is_open())
    {
