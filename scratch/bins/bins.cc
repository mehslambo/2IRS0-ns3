...
// !!!!!!!!!!!!!!!!!!!!!!!
// Simulation parameters line 42~51
// configureTCPSensorClients is the only interesting function
// Node placement starts at line 883
// Channel construction starts at line 909
...
Time stopTime = Minutes(5);

uint32_t gateways = 1;
uint32_t totalNodes = 87;
uint32_t perAxis = 10;

uint32_t packetSize = 1;
uint32_t totalSize = 1;

uint32_t areaSize = 3484;
  ...
  uint32_t tql=0;
	for (uint32_t i = 0; i < totalNodes; i++) {
		...
		stats.get(i).EDCAQueueLength = nodes[i]->queueLength;
                tql+=nodes[i]->queueLength;
	}
        cout << "Total queue length = " << tql << endl;
...
int main(int argc, char *argv[]) {
  ...
  for (NodeContainer::Iterator j = wifiStaNode.Begin (); j != wifiStaNode.End (); ++j)
    {
		//Place nodes via the same idea as the LoRaWAN placements
  ...
  {
    wifiApNode.Get(i)->GetObject<MobilityModel>()->SetPosition(Vector(0.5*areaSize, 0.5*areaSize,20));
  }
	...
	channelBuilder.AddPropagationLoss("ns3::OkumuraHataPropagationLossModel", 
    "Frequency", DoubleValue(868000000),
	...
	phy.Set("EnergyDetectionThreshold", DoubleValue(-130.0)); //CHANGING THIS DOES NOT DO ANYTHING
	phy.Set("CcaMode1Threshold", DoubleValue(-130.0));  ///CHANGING THIS DOES NOT DO ANYTHING
